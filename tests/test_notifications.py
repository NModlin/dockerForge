...
from src.notifications.notification_manager import (
    NotificationManager, Notification, NotificationSeverity, NotificationType
)
from src.notifications.preference_manager import PreferenceManager, UserPreferences
from src.notifications.template_manager import TemplateManager, NotificationTemplate
from src.fixes.fix_proposal import FixProposalManager, FixProposal, FixStep, FixRiskLevel, FixStatus
...
class TestNotificationManager:
    ...
    def notification_manager(self):
        ...
        with patch("src.notifications.notification_manager.get_config") as mock_get_config:
            # Mock configuration
    ...
    def test_get_notifications(self, notification_manager):
        ...
        notifications = notification_manager.get_notifications(severity=NotificationSeverity.INFO)
        assert len(notifications) == 1
        assert notifications[0].severity == NotificationSeverity.INFO
        
        # Get notifications by type
        notifications = notification_manager.get_notifications(notification_type=NotificationType.CONTAINER_EXIT)
        assert len(notifications) == 1
        assert notifications[0].notification_type == NotificationType.CONTAINER_EXIT
        
        # Get notifications by container
        notifications = notification_manager.get_notifications(container_id="test-container")
        assert len(notifications) == 1
    ...
    def test_acknowledge_notification(self, notification_manager):
        ...
        result = notification_manager.acknowledge_notification(notification.id, "test-user")
...
class TestPreferenceManager:
    ...
    def preference_manager(self):
        ...
        with patch("src.notifications.preference_manager.get_config") as mock_get_config:
            # Mock configuration
            mock_get_config.return_value = tempfile.mkdtemp()
    ...
    def test_should_notify(self):
        ...
        assert preferences.should_notify(NotificationSeverity.INFO, NotificationType.CUSTOM) is False
        assert preferences.should_notify(NotificationSeverity.WARNING, NotificationType.CUSTOM) is True
        assert preferences.should_notify(NotificationSeverity.ERROR, NotificationType.CUSTOM) is True
        assert preferences.should_notify(NotificationSeverity.CRITICAL, NotificationType.CUSTOM) is True
        
        # Test notification types
        preferences.notification_types["custom"] = False
        assert preferences.should_notify(NotificationSeverity.WARNING, NotificationType.CUSTOM) is False
        
        # Test container filters
        preferences.notification_types["custom"] = True
        preferences.container_filters = ["test-container"]
        assert preferences.should_notify(NotificationSeverity.WARNING, NotificationType.CUSTOM, "test-container") is True
        assert preferences.should_notify(NotificationSeverity.WARNING, NotificationType.CUSTOM, "other-container") is False
...
class TestTemplateManager:
    ...
    def test_create_notification_template(self):
        ...
        assert template.html_template == "<html><body><h1>{{ title }}</h1><p>{{ message }}</p></body></html>"
    ...
    def test_render_template(self):
        ...
        # Check HTML
        assert html == "<html><body><h1>Test Title</h1><p>Test Message</p></body></html>"
    ...
    def test_render_template_manager(self, template_manager):
        ...
        # Render template
        result = template_manager.render_template("test-template", "html", {
            "title": "Test Title",
            "message": "Test Message",
        })
        
        # Check result
        assert result == "<html><body><h1>Test Title</h1><p>Test Message</p></body></html>"
...
class TestFixProposalManager:
    ...
    def fix_proposal_manager(self):
        ...
        with patch("src.fixes.fix_proposal.get_config") as mock_get_config:
            ...
            with patch("src.fixes.fix_proposal.get_notification_manager") as mock_get_notification_manager:
