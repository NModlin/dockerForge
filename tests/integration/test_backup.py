...
class TestBackupFunctionality:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "ps",
                    "--filter",
                    f"name={self.test_container_name}",
                    "--format",
                    "{{.Names}}",
                ],
                check=True,
    ...
    def test_image_import(self):
        ...
        try:
            ...
            subprocess.run(
                ["docker", "rmi", import_tag], check=False, capture_output=True
            )
        except (ImportError, NameError):
            pytest.skip("ExportImport not available in this environment")
        except Exception as e:
    ...
    def test_container_export(self):
        ...
        try:
            ...
            success = export_import.export_container(
                self.test_container_name, export_path
            )
            assert success is True
            # Check that the export file was created
            assert os.path.exists(export_path)
