...
class TestPerformance:
    ...
    def test_api_response_time(self):
        ...
        for endpoint in endpoints:
            ...
            ), f"Average response time for {endpoint} is too high: {avg_response_time:.3f} seconds"
    ...
    def test_concurrent_requests(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            response_times = list(
                executor.map(lambda _: make_request(), range(num_concurrent))
            )
        ...
        assert (
            avg_response_time < max_response_time
        ), f"Average response time for concurrent requests to {endpoint} is too high: {avg_response_time:.3f} seconds"
    ...
    def test_database_performance(self):
        ...
        for _ in range(num_requests):
            ...
            response = self.client.get(
                endpoint, params={"hours": 24}, headers=self.headers
            )
            end_time = time.time()
        ...
        assert (
            avg_response_time < max_response_time
        ), f"Average response time for database operations is too high: {avg_response_time:.3f} seconds"
