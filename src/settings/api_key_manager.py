...
class ApiKeyManager:
    ...
    def get_key_usage_stats(self, key_id: int, db: Session) -> Dict[str, Any]:
        ...
        usage_records = (
            db.query(ApiKeyUsage).filter(ApiKeyUsage.api_key_id == key_id).all()
        )
        ...
        requests_today = sum(
            1 for record in usage_records if record.created_at.date() == today
        )

        # Calculate success rate
        successful_requests = sum(
            1 for record in usage_records if 200 <= record.status_code < 300
        )
        success_rate = (
            round((successful_requests / total_requests) * 100)
            if total_requests > 0
            else 0
        )
        ...
        avg_response_time = (
            round(
                sum(record.response_time for record in usage_records) / total_requests,
                2,
            )
            if total_requests > 0
            else 0
    ...
    def _calculate_usage_over_time(
        ...
        while current_date <= end_date:
            result.append({"date": current_date.strftime("%Y-%m-%d"), "count": 0})
