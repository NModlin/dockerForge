...
class FixApplier:
    ...
    def _execute_command(self, command: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Command failed: {command}, returncode: {process.returncode}"
                )
    ...
    def _modify_file(self, file_path: str, code: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if not backup_path and os.path.exists(file_path):
                logger.warning(
                    f"Failed to backup file {file_path}, aborting modification"
                )
    ...
    def apply_fix(self, fix_id: str, dry_run: bool = None) -> Dict[str, Any]:
        ...
        for i, step in enumerate(fix.steps):
            ...
            if dry_run:
                ...
                if step.command:
                    ...
                    step_result["success"] = True
                    logger.info(
                        f"Dry run: Would require manual action: {step.manual_action}"
                    )
                else:
                ...
                if success and step.verification:
                    ...
                    if "command" in verify_details:
                        step_result["verification"]["command"] = verify_details[
                            "command"
                        ]
                    ...
                    if not verify_success:
                        ...
                        # Add error message
                        if "error" in verify_details:
                            step_result["verification"]["error"] = verify_details[
                                "error"
                            ]
        ...
        if not dry_run:
            if results["success"]:
                self._fix_proposal_manager.update_fix_status(
                    fix_id, FixStatus.APPLIED, results
                )
                logger.info(f"Fix applied successfully: {fix_id} - {fix.title}")
            else:
                self._fix_proposal_manager.update_fix_status(
                    fix_id, FixStatus.FAILED, results
                )
    ...
    def rollback_fix(self, fix_id: str) -> Dict[str, Any]:
        ...
        for i, step_result in reversed(list(enumerate(fix.result.get("steps", [])))):
            ...
            # Log step
            logger.info(
                f"Rolling back step {i+1}/{len(fix.result['steps'])}: {rollback_result['title']}"
            )
            ...
            if not step_result.get("success", False):
                ...
                logger.info(
                    f"Skipping rollback of step {i+1}: Step was not successfully applied"
                )
            ...
            if "file_path" in step_result and "backup_path" in step_result:
                ...
                try:
                    ...
                    if not os.path.exists(backup_path):
                        ...
                        rollback_result["success"] = True
                        logger.info(
                            f"Restored file {file_path} from backup {backup_path}"
                        )
                except Exception as e:
                    ...
                    results["success"] = False
                    logger.error(
                        f"Error restoring file {file_path} from backup {backup_path}: {str(e)}"
                    )
                ...
                rollback_result["success"] = True
                rollback_result["note"] = (
                    "Command execution cannot be automatically rolled back"
                )
                logger.info(
                    f"Command execution cannot be automatically rolled back: {step_result['command']}"
                )
            else:
                # No rollback needed
                rollback_result["skipped"] = True
        ...
        if results["success"]:
            self._fix_proposal_manager.update_fix_status(
                fix_id, FixStatus.ROLLED_BACK, results
            )
