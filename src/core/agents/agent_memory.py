...
class FileMemoryStore(MemoryStore):
    ...
    def __init__(self, agent_id: str, name: str = "default", base_dir: Optional[str] = None):
        ...
        logger.debug(f"Initialized FileMemoryStore for agent {agent_id} at {self.agent_dir}")
...
class SQLiteMemoryStore(MemoryStore):
    ...
    def __init__(self, agent_id: str, name: str = "default", db_path: Optional[str] = None):
        super().__init__(agent_id, name)
...
class AgentMemory:
    ...
    def __init__(self, agent_id: str, store_type: str = "sqlite"):
        ...
        logger.debug(f"Initialized AgentMemory for agent {agent_id} using {store_type} storage")
    ...
    def list_episodes(self, 
                     ...
                     limit: int = 100) -> List[Tuple[str, MemoryEntry]]:
        """
    ...
    def save_semantic(self, key: str, content: Any, metadata: Optional[Dict[str, Any]] = None) -> None:
    ...
    def import_from(self, export_data: Dict[str, Any]) -> None:
        ...
        if export_data.get("agent_id") != self.agent_id:
            logger.warning(f"Importing from different agent ID: {export_data.get('agent_id')}")
