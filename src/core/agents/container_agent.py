...
class ContainerCommand:
    ...
    @staticmethod
    async def run_command(command: List[str], timeout: float = 30.0) -> Tuple[int, str, str]:
        """
    ...
    @staticmethod
    async def list_containers(all_containers: bool = False) -> Tuple[bool, List[Dict[str, Any]]]:
        """
    ...
    @staticmethod
    async def get_container_info(container_id: str) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """
    ...
    @staticmethod
    async def get_container_logs(container_id: str, tail: Optional[int] = None) -> Tuple[bool, str]:
        """
    ...
    @staticmethod
    async def restart_container(container_id: str, timeout: int = 10) -> Tuple[bool, str]:
        """
        Restart a container
    ...
    @staticmethod
    async def remove_container(container_id: str, force: bool = False) -> Tuple[bool, str]:
        """
        Remove a container
    ...
    @staticmethod
    async def exec_in_container(container_id: str, command_args: List[str]) -> Tuple[bool, str]:
        """
...
class ContainerAgent(Agent):
    ...
    async def _handle_list_containers(self, task: AgentTask, context: AgentContext) -> AgentResult:
    ...
    async def _handle_inspect_container(self, task: AgentTask, context: AgentContext) -> AgentResult:
        ...
        success, container_info = await ContainerCommand.get_container_info(container_id)
        ...
        # Store result in memory for future reference
        timestamp = datetime.now().isoformat()
        self.memory.save_state(f"container_info_{container_id}_{timestamp}", container_info)
    ...
    async def _handle_container_logs(self, task: AgentTask, context: AgentContext) -> AgentResult:
    ...
    async def _handle_start_container(self, task: AgentTask, context: AgentContext) -> AgentResult:
    ...
    async def _handle_stop_container(self, task: AgentTask, context: AgentContext) -> AgentResult:
    ...
    async def _handle_restart_container(self, task: AgentTask, context: AgentContext) -> AgentResult:
        ...
        success, message = await ContainerCommand.restart_container(container_id, timeout)
    ...
    async def _handle_remove_container(self, task: AgentTask, context: AgentContext) -> AgentResult:
    ...
    async def _handle_exec_in_container(self, task: AgentTask, context: AgentContext) -> AgentResult:
        ...
        success, output = await ContainerCommand.exec_in_container(container_id, command)
    ...
    async def _handle_diagnose_container(self, task: AgentTask, context: AgentContext) -> AgentResult:
        ...
        self.memory.save_state(f"container_diagnosis_{container_id}_{diagnosis['timestamp']}", diagnosis)
    ...
    async def _handle_fix_container(self, task: AgentTask, context: AgentContext) -> AgentResult:
        ...
        if not issue_type:
            return AgentResult(
                success=False,
                task_id=task.id,
                error="Missing issue_type parameter"
            )
        ...
        diagnosis_task = AgentTask(
            ...
            input_data={"task_type": "container.diagnose", "container_id": container_id}
        )
        diagnosis_result = await self._handle_diagnose_container(diagnosis_task, context)
        ...
        if issue_type == "high_restart_count" or issue_type == "non_zero_exit":
            ...
            restart_task = AgentTask(
                ...
                input_data={"task_type": "container.restart", "container_id": container_id}
            )
        ...
        verification_result = await self._handle_diagnose_container(verification_task, context)
