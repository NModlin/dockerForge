...
class ExecutionEngine:
    ...
    async def execute_task(self, 
                         ...
                         progress_callback: Optional[Callable[[float], None]] = None) -> AgentResult:
        """
        ...
        context.logger.info(f"Starting execution of task {task_id} with agent {context.agent.agent_id}")
        ...
        try:
            ...
            try:
                ...
                context.logger.info(f"Executing task with timeout of {timeout_seconds} seconds")
                ...
                context.task.status = TaskStatus.COMPLETED if context.result.success else TaskStatus.FAILED
                context.task.error = context.result.error
                context.task.output_data = context.result.data
                context.task.progress = 1.0 if context.result.success else context.task.progress
                ...
                context.logger.error(f"Task execution timed out after {timeout_seconds} seconds")
            ...
            context.stats.cpu_time = (context.stats.end_time - context.stats.start_time).total_seconds()
            
            # Update error count from logs
            context.stats.error_count = len([log for log in context.logger.logs if log["level"] == "ERROR"])
            context.stats.warning_count = len([log for log in context.logger.logs if log["level"] == "WARNING"])
    ...
    def get_active_tasks(self) -> List[Dict[str, Any]]:
        ...
        for task_id, context in self._active_executions.items():
            result.append({
                ...
                "start_time": context.stats.start_time.isoformat() if context.stats.start_time else None,
    ...
    def get_task_logs(self, 
        ...
        if task_id in self._active_executions:
            return self._active_executions[task_id].logger.get_logs(level, limit, offset)
