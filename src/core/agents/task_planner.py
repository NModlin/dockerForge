...
class TaskPlan:
    ...
    def get_related_tasks(self, 
                        task_id: str,
                        relation_type: Optional[TaskRelationType] = None) -> List[AgentTask]:
        ...
        for relation in self.relations:
            if relation["from_task_id"] == task_id:
                if relation_type is None or relation["relation_type"] == relation_type.value:
    ...
    def get_dependencies(self, task_id: str) -> List[AgentTask]:
        ...
        for relation in self.relations:
            if relation["from_task_id"] == task_id and relation["relation_type"] == TaskRelationType.DEPENDS_ON.value:
    ...
    def get_dependents(self, task_id: str) -> List[AgentTask]:
        ...
        for relation in self.relations:
            if relation["to_task_id"] == task_id and relation["relation_type"] == TaskRelationType.DEPENDS_ON.value:
    ...
    def get_completion_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_complete": completed_tasks + failed_tasks + cancelled_tasks == total_tasks,
            "is_successful": failed_tasks == 0 and cancelled_tasks == 0 and completed_tasks == total_tasks
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TaskPlan":
        ...
        plan.created_at = datetime.fromisoformat(data.get("created_at", 
    ...
    def _would_create_cycle(self, 
        ...
        def has_path(start: str, end: str) -> bool:
            ...
            for relation in self.relations:
                if relation["from_task_id"] == start and relation["relation_type"] == TaskRelationType.DEPENDS_ON.value:
...
class TaskPlanner:
    ...
    def decompose_task(self, 
                      task: AgentTask, 
                      subtasks: List[Dict[str, Any]]) -> TaskPlan:
        """
    ...
    def update_plan_from_task(self, 
                             plan_id: str, 
                             task: AgentTask) -> bool:
