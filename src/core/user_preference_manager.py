...
class UserPreferenceManager:
    ...
    def get_user_preferences(self, user_id: int, db_session=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if db_session is None:
                logger.warning(
                    "No database session provided, returning default preferences"
                )
            ...
            if preferences is None:
                preferences = UserPreference(
                    ...
                    feedback_preferences=self.default_preferences[
                        "feedback_preferences"
                    ],
                )
            ...
            return {
                ...
                or self.default_preferences["feedback_preferences"],
    ...
    def update_user_preferences(
        self, user_id: int, preferences_data: Dict[str, Any], db_session=None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if db_session is None:
                logger.warning(
                    "No database session provided, skipping preference update"
                )
    ...
    def process_message_feedback(
        ...
        try:
            ...
            if db_session is None:
                logger.warning(
                    "No database session provided, skipping feedback processing"
                )
                return {"success": False, "error": "No database session provided"}
            ...
            if message:
                ...
                if preferences.feedback_preferences is None:
                    preferences.feedback_preferences = self.default_preferences[
                        "feedback_preferences"
                    ]
                ...
                topic_ratings = preferences.feedback_preferences.get(
                    "topic_ratings", {}
                )
                for topic in topics:
                    ...
                    topic_ratings[topic]["avg_rating"] = (
                        topic_ratings[topic]["total_rating"]
                        / topic_ratings[topic]["count"]
                    )

                # Update rating counts
                if rating >= 4:
                    preferences.feedback_preferences["positively_rated_responses"] = (
                        preferences.feedback_preferences.get(
                            "positively_rated_responses", 0
                        )
                        + 1
                    )
                ...
                # Update timestamp
                preferences.feedback_preferences["last_feedback_time"] = (
    ...
    def get_user_command_shortcuts(
        self, user_id: int, db_session=None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if db_session is None:
                logger.warning(
                    "No database session provided, returning empty shortcuts list"
                )
                return []
            ...
            shortcuts = (
                db_session.query(ChatCommandShortcut)
                .filter(ChatCommandShortcut.user_id == user_id)
                .order_by(ChatCommandShortcut.usage_count.desc())
                .all()
            )
    ...
    def create_command_shortcut(
        ...
        try:
            ...
            if db_session is None:
                logger.warning(
                    "No database session provided, skipping shortcut creation"
                )
                return {"success": False, "error": "No database session provided"}
    ...
    def use_command_shortcut(
        self, user_id: int, command: str, db_session=None
    ) -> Dict[str, Any]:
        """
    ...
    def delete_command_shortcut(
        self, user_id: int, shortcut_id: int, db_session=None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if db_session is None:
                logger.warning(
                    "No database session provided, skipping shortcut deletion"
                )
                return {"success": False, "error": "No database session provided"}
    ...
    def _learn_from_feedback(self, preferences, topics: List[str], rating: int) -> None:
        ...
        if rating >= 4:
            for topic in topics:
                if (
                    topic not in preferences.preferred_topics
                    and topic not in preferences.avoided_topics
                ):
