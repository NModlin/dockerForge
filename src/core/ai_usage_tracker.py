...
class AIUsageTracker:
    ...
    def record_usage(
        ...
        try:
            ...
            logger.debug(
                f"Recorded usage: {provider}/{model} {operation} - ${cost_usd:.4f}"
            )
        except sqlite3.Error as e:
    ...
    def set_monthly_budget(
        self, year: int, month: int, provider: str, budget_usd: float
    ):
        """
    ...
    def check_budget_status(self) -> Dict[str, Any]:
        ...
        if usage["total_budget_usd"] > 0:
            status["total_percentage"] = (
                usage["total_cost_usd"] / usage["total_budget_usd"]
            ) * 100
    ...
    def is_within_budget(self, provider: str, estimated_cost: float) -> bool:
        ...
        if provider in monthly_usage["providers"]:
            provider_monthly_usage = monthly_usage["providers"][provider][
                "total_cost_usd"
            ]
    ...
    def get_usage_report(self) -> Dict[str, Any]:
        ...
        projected_total = monthly_usage["total_cost_usd"] + (
            daily_average * days_remaining
        )

        # Calculate budget status
        budget_remaining = (
            monthly_usage["total_budget_usd"] - monthly_usage["total_cost_usd"]
        )
        budget_percentage = 0.0
        if monthly_usage["total_budget_usd"] > 0:
            budget_percentage = (
                monthly_usage["total_cost_usd"] / monthly_usage["total_budget_usd"]
            ) * 100
        ...
        if monthly_usage["total_budget_usd"] > 0:
            report["projected_percentage"] = (
                projected_total / monthly_usage["total_budget_usd"]
            ) * 100
