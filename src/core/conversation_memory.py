...
class ConversationMemoryManager:
    ...
    def get_relevant_memories(
        ...
        try:
            ...
            if db_session is None:
                logger.warning(
                    "No database session provided, skipping memory retrieval"
                )
                return []
            ...
            if query_embedding is None:
                ...
                memories = (
                    db_session.query(ConversationMemory)
            ...
            for memory in memories:
                if memory.embedding is not None:
                    ...
                    memory_scores.append({"memory": memory, "score": combined_score})
    ...
    def _extract_key_information(
        ...
        key_info = {"topics": [], "entities": [], "intent": "unknown"}
        ...
        if "?" in text:
            ...
            for command in ["start", "run", "create", "build", "deploy"]
        ):
            key_info["intent"] = "command"
        elif any(
            keyword in text_lower
            for keyword in ["error", "issue", "problem", "fail", "help"]
        ):
    ...
    def _prune_old_memories(self, user_id: int, db_session) -> None:
        ...
        try:
            ...
            if memory_count > self.memory_limit:
                ...
                to_delete = (
                    db_session.query(ConversationMemory.id)
                    .filter(ConversationMemory.user_id == user_id)
                    .order_by(ConversationMemory.importance_score.asc())
                    .limit(excess_count)
                    .all()
                )
                ...
                if to_delete:
                    ...
                    logger.info(
                        f"Pruned {len(delete_ids)} old memories for user {user_id}"
                    )
