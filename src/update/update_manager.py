...
class UpdateManager:
    ...
    def __init__(
        self, config_manager: ConfigManager, platform_adapter: PlatformAdapter
    ):
        """
    ...
    def _restore_from_backup(self, backup_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Restoring from backup created at {metadata.get('timestamp')} for version {metadata.get('version')}"
            )
            ...
            if os.path.exists(config_backup_path):
                config_dir = os.path.dirname(
                    self.config_manager.config_path or "./config"
                )
    ...
    def _migrate_configuration(self, from_version: str, to_version: str) -> bool:
        ...
        try:
            logger.info(
                f"Migrating configuration from version {from_version} to {to_version}"
            )
    ...
    def update(self, version: Optional[str] = None, force: bool = False) -> bool:
        ...
        try:
            ...
            if not version:
                update_available, latest_version, _ = (
                    self.version_checker.check_for_updates(force=True)
                )
            ...
            if not self._migrate_configuration(current_version, version):
                logger.warning(
                    "Configuration migration had issues, but update completed"
                )
    ...
    def _update_via_docker(self, version: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"To update to version {version}, pull the new image and restart the container:"
            )
    ...
    def rollback(self, backup_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not current_backup:
                logger.warning(
                    "Failed to create backup of current state before rollback"
                )
            ...
            logger.info(
                f"Successfully rolled back to backup: {os.path.basename(backup_path)}"
            )
            return True
