...
class ContainerLogCollector:
    ...
    def __init__(
        ...
        self.last_timestamp = datetime.now() - timedelta(
            hours=1
        )  # Start with logs from the last hour
    ...
    def _collect_logs(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error collecting logs from container {self.container_name}: {str(e)}"
            )
        except Exception as e:
            logger.exception(
                f"Unexpected error collecting logs from container {self.container_name}: {str(e)}"
            )
        finally:
            self.running = False
            ...
            if " " in line_str:
                ...
                try:
                    ...
                    timestamp = datetime.fromisoformat(
                        timestamp_str.rstrip("Z").replace("T", " ")
                    )
                except ValueError:
        ...
        with self.lock:
            ...
            for container_id in current_ids - new_ids:
                ...
                if collector:
                    collector.stop()
                    logger.debug(
                        f"Stopped log collection for container {collector.container_name}"
                    )
            ...
            for container in containers:
                if container.id not in self.collectors:
                    ...
                    logger.debug(
                        f"Started log collection for container {container.name}"
                    )
