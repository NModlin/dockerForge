...
class IssueDetector:
    ...
    def __init__(self):
        ...
        self.issues_dir = get_config(
            "monitoring.issues_dir", os.path.expanduser("~/.dockerforge/issues")
        )
    ...
    def _create_issue_from_pattern_match(self, match: PatternMatch) -> Optional[Issue]:
        ...
        for issue in self.issues.values():
            if (
                issue.container_id == match.log_entry.container_id
                and issue.pattern_id == match.pattern_id
                and issue.status
        ...
        # Create new issue
        issue_id = (
            f"{match.log_entry.container_id[:8]}_{match.pattern_id}_{int(time.time())}"
        )
    ...
    def create_issue_from_analysis(
        self, analysis_result: AnalysisResult, issue_data: Dict[str, Any]
    ) -> Issue:
        """
