...
class PatternDatabase:
    ...
    def __init__(self, patterns_dir: Optional[str] = None):
        ...
        self.patterns_dir = patterns_dir or get_config(
            "monitoring.patterns_dir", os.path.expanduser("~/.dockerforge/patterns")
        )
            ...
            for filename in os.listdir(self.patterns_dir):
                if filename.endswith(".json"):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error loading pattern from {file_path}: {str(e)}"
                        )
        except Exception as e:
    ...
    def add_pattern(self, pattern: PatternDefinition) -> None:
        ...
        if pattern.id in self.patterns:
            logger.warning(
                f"Pattern with ID '{pattern.id}' already exists, overwriting"
            )
