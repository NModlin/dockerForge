...
from src.monitoring.log_collector import LogEntry, LogFilter, get_log_collection_manager
...
class LogStatistics:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "LogStatistics":
        ...
        """
        data = data.copy()
        data["time_range"] = tuple(
            datetime.fromisoformat(t) for t in data["time_range"]
        )
...
class LogExplorer:
    ...
    def get_log_statistics(
        ...
        warning_count = sum(
            1 for log in logs if re.search(r"(?i)(warning|warn|caution)", log.message)
        )
    ...
    def extract_log_context(
        ...
        try:
            index = next(
                ...
                if (
                    log.container_id == log_entry.container_id
                    and log.timestamp == log_entry.timestamp
                    and log.message == log_entry.message
                )
            )
    ...
    def export_logs(
        ...
        if format == "text":
            ...
            # Write header
            writer.writerow(
                ["Container ID", "Container Name", "Timestamp", "Stream", "Message"]
            )
    ...
    def analyze_log_trends(
        ...
        analysis = {
            ...
            "pattern_timelines": {
                pid: dict(ptl) for pid, ptl in pattern_timelines.items()
            },
