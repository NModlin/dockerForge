...
def setup_backup_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "backup", help="Backup Docker containers, images, and volumes"
    )

    # Set up subparsers for backup commands
    backup_subparsers = parser.add_subparsers(
        title="backup commands", dest="backup_command", help="Backup command to run"
    )
    backup_subparsers.required = True

    # Backup container
    container_parser = backup_subparsers.add_parser(
        "container", help="Backup a Docker container"
    )
    container_parser.add_argument(
        "container", help="ID or name of the container to backup"
    )
    container_parser.add_argument(
        "--name", help="Name of the backup. If not provided, a name will be generated."
    )
    container_parser.add_argument(
        "--no-volumes", action="store_true", help="Don't include volumes in the backup"
    )
    container_parser.add_argument(
        "--include-image",
        action="store_true",
    ...
    # Show backup
    show_parser = backup_subparsers.add_parser("show", help="Show backup details")
    show_parser.add_argument("backup_id", help="ID of the backup to show")
...
def setup_restore_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "restore", help="Restore Docker containers, images, and volumes from backup"
    )
    parser.add_argument("backup_id", help="ID of the backup to restore")
    parser.add_argument(
        "--name",
        help="Name for the restored container. If not provided, a name will be generated.",
    )
    parser.add_argument(
        "--no-volumes", action="store_true", help="Don't restore volumes"
    )
    parser.add_argument(
        "--no-image", action="store_true", help="Don't restore the container's image"
    )
...
def setup_export_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "export", help="Export Docker containers, images, and volumes to files"
    )

    # Set up subparsers for export commands
    export_subparsers = parser.add_subparsers(
        title="export commands", dest="export_command", help="Export command to run"
    )
    export_subparsers.required = True

    # Export image
    image_parser = export_subparsers.add_parser(
        "image", help="Export a Docker image to a file"
    )
    image_parser.add_argument("image", help="ID or name of the image to export")
    image_parser.add_argument(
        "--output",
        help="Path to save the exported image. If not provided, a default path will be used.",
    )
    image_parser.add_argument(
        "--no-compress", action="store_true", help="Don't compress the exported image"
    )
    image_parser.set_defaults(func=handle_export_image)

    # Export container
    container_parser = export_subparsers.add_parser(
        "container", help="Export a Docker container to a file"
    )
    container_parser.add_argument(
        "container", help="ID or name of the container to export"
    )
    container_parser.add_argument(
        "--output",
    ...
    volume_parser = export_subparsers.add_parser(
        "volume", help="Export a Docker volume to a file"
    )
    volume_parser.add_argument("volume", help="Name of the volume to export")
    volume_parser.add_argument(
        "--output",
        help="Path to save the exported volume. If not provided, a default path will be used.",
    )
    volume_parser.add_argument(
        "--no-compress", action="store_true", help="Don't compress the exported volume"
    )
...
def setup_import_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "import", help="Import Docker containers, images, and volumes from files"
    )

    # Set up subparsers for import commands
    import_subparsers = parser.add_subparsers(
        title="import commands", dest="import_command", help="Import command to run"
    )
    import_subparsers.required = True

    # Import image
    image_parser = import_subparsers.add_parser(
        "image", help="Import a Docker image from a file"
    )
    image_parser.add_argument("file", help="Path to the image file to import")
    image_parser.add_argument(
        "--repository", help="Repository name for the imported image"
    )
    image_parser.add_argument("--tag", help="Tag for the imported image")
    image_parser.set_defaults(func=handle_import_image)

    # Import container
    container_parser = import_subparsers.add_parser(
        "container", help="Import a Docker container from a file"
    )
    container_parser.add_argument("file", help="Path to the container file to import")
    container_parser.add_argument(
        "--repository", help="Repository name for the imported container image"
    )
    container_parser.add_argument("--tag", help="Tag for the imported container image")
    container_parser.add_argument("--name", help="Name for the imported container")
    container_parser.set_defaults(func=handle_import_container)

    # Import volume
    volume_parser = import_subparsers.add_parser(
        "volume", help="Import a Docker volume from a file"
    )
    volume_parser.add_argument("file", help="Path to the volume file to import")
    volume_parser.add_argument("--name", help="Name for the imported volume")
...
def handle_list_backups(args):
    ...
    try:
        ...
        if args.format == "json":
            ...
            # Print header
            print(
                f"{'ID':<30} {'Container':<30} {'Timestamp':<25} {'Volumes':<10} {'Image':<10}"
            )
            print("-" * 105)
...
def handle_show_backup(args):
    ...
    try:
        ...
        if args.format == "json":
            ...
            print(
                f"Include Volumes: {'Yes' if backup.get('include_volumes') else 'No'}"
            )
            ...
            if backup.get("include_volumes") and backup.get("volumes"):
                ...
                for volume in backup.get("volumes", []):
                    print(
                        f"  {volume.get('volume_name', '')}: {volume.get('mount_point', '')}"
                    )
...
def handle_restore(args):
    ...
    try:
        ...
        print(
            f"Container restored: {result['restored_container_name']} ({result['restored_container_id']})"
        )
...
def handle_import_image(args):
    ...
    try:
        ...
        result = export_import_manager.import_image(
            input_path=args.file, repository=args.repository, tag=args.tag
        )
...
def handle_import_volume(args):
    ...
    try:
        ...
        result = export_import_manager.import_volume(
            input_path=args.file, volume_name=args.name
        )
