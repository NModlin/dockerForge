...
class CheckpointManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error loading checkpoint config from {self.config_file}, creating new one"
            )
            self._init_config()
            return {"checkpoints": []}
    ...
    def _backup_config_file(
        self, file_path: str, dest_dir: str
    ...
    def delete_checkpoint(self, checkpoint_id: str) -> bool:
        ...
        try:
            ...
            config = self._load_config()
            config["checkpoints"] = [
                c for c in config["checkpoints"] if c["checkpoint_id"] != checkpoint_id
            ]
            self._save_config(config)
    ...
    def compare_checkpoints(
        ...
        if (
            compare_containers
            and checkpoint1.get("include_containers")
            and checkpoint2.get("include_containers")
        ):
            ...
            containers_comparison = self._compare_items(
                containers1, containers2, "id", ["status"]
            )
            ...
            comparison["summary"]["unchanged"] += len(
                containers_comparison["unchanged"]
            )

        # Compare volumes
        if (
            compare_volumes
            and checkpoint1.get("include_volumes")
            and checkpoint2.get("include_volumes")
        ):
        ...
        if (
            compare_configs
            and checkpoint1.get("include_configs")
            and checkpoint2.get("include_configs")
        ):
            configs1 = self._load_checkpoint_items(checkpoint_id_1, "configs")
            configs2 = self._load_checkpoint_items(checkpoint_id_2, "configs")

            configs_comparison = self._compare_configs(
                configs1, configs2, checkpoint_id_1, checkpoint_id_2
            )
    ...
    def _load_checkpoint_items(
        self, checkpoint_id: str, item_type: str
    ) -> List[Dict[str, Any]]:
    ...
    def restore_from_checkpoint(
        ...
        result = {"checkpoint_id": checkpoint_id, "restored_items": {}, "errors": []}
        ...
        if restore_containers and checkpoint.get("include_containers"):
            ...
            for container_info in containers:
                try:
                    ...
                    environment = (
                        container_info["env"] if "env" in container_info else []
                    )
                    ports = {}
                            ...
                            if bindings:
                                for binding in bindings:
                                    ...
                                    ports[container_port] = (
                                        (host_ip, host_port) if host_ip else host_port
                                    )

                    volumes = (
                        container_info["volumes"] if "volumes" in container_info else []
                    )
                    network_mode = (
                        container_info["network_mode"]
                        if "network_mode" in container_info
                        else None
                    )
                    ...
                    container = self.docker_client.containers.run(
                        ...
                        restart_policy=container_info.get(
        ...
        if restore_volumes and checkpoint.get("include_volumes"):
            ...
            for volume_info in volumes:
                try:
                    ...
                    try:
                        existing_volume = self.docker_client.volumes.get(
                            volume_info["name"]
                        )
                    except docker.errors.NotFound:
                        pass
                    ...
                    restored_volumes.append(
                        {"name": volume.name, "driver": volume.attrs["Driver"]}
                    )
            ...
            for config_info in configs:
                try:
                    ...
                    checkpoint_config_path = os.path.join(
                        checkpoint_path, "configs", config_info["file_name"]
                    )
