...
class ExportImportManager:
    ...
    def __init__(self):
        ...
        self.export_dir = self.config.get("export_dir", os.path.expanduser("~/.dockerforge/exports"))
    ...
    def export_image(
        ...
        try:
            ...
            if not output_path:
                ...
                filename = f"{image_name.replace('/', '_').replace(':', '_')}_{timestamp}.tar"
                if compress:
                    filename += ".gz"
            ...
            if input_path.endswith(".gz"):
                ...
                try:
                    ...
                    if not extracted_files:
                        raise ValueError(f"No files found in compressed archive: {input_path}")
                    
                    # Use first extracted file
                    extracted_path = os.path.join(os.path.dirname(temp_path), extracted_files[0])
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not os.path.exists(fs_path):
                    raise ValueError(f"Container filesystem not found in archive: {input_path}")
                
                if not os.path.exists(config_path):
                    raise ValueError(f"Container config not found in archive: {input_path}")
                ...
                image_tag = f"{repository}:{tag}" if repository and tag else "imported:latest"
