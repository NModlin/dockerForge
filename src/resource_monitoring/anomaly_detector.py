...
class AnomalyDetector:
    ...
    def __init__(self, config_manager: ConfigManager, metrics_collector: MetricsCollector, 
        ...
        self.anomaly_config = self.config.get('resource_monitoring', {}).get('anomaly_detection', {})
        self.detection_interval = self.anomaly_config.get('detection_interval', 300)  # seconds
        self.lookback_period = self.anomaly_config.get('lookback_period', 24)  # hours
        self.threshold_multiplier = self.anomaly_config.get('threshold_multiplier', 3.0)  # standard deviations
        self.min_data_points = self.anomaly_config.get('min_data_points', 10)  # minimum data points for detection
        self.enabled_metrics = self.anomaly_config.get('enabled_metrics', {
            'cpu': True,
            'memory': True,
            'disk': True,
            'network': True
    ...
    def detect_anomalies(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for metric_type, enabled in self.enabled_metrics.items():
            ...
            for container_id, container_metrics in metrics.items():
                if metric_type not in container_metrics or not container_metrics[metric_type]:
                    continue
                    
                # Detect anomalies for this container and metric type
                anomalies = self._detect_container_anomalies(container_id, metric_type, container_metrics[metric_type])
    ...
    def _detect_container_anomalies(self, container_id: str, metric_type: str, 
        ...
        # Statistical outlier detection
        outlier_anomalies = self._detect_statistical_outliers(container_id, metric_type, timestamps, values, baseline)
        anomalies.extend(outlier_anomalies)
        
        # Trend analysis
        trend_anomalies = self._detect_trend_anomalies(container_id, metric_type, timestamps, values, baseline)
        anomalies.extend(trend_anomalies)
        
        # Seasonality analysis (if enough data)
        if len(values) >= 24:  # At least 24 data points for hourly seasonality
            seasonality_anomalies = self._detect_seasonality_anomalies(container_id, metric_type, timestamps, values, baseline)
            anomalies.extend(seasonality_anomalies)
            
        return anomalies
        
    def _update_baseline(self, container_id: str, metric_type: str, metrics_data: List[Dict[str, Any]]) -> None:
        """
    ...
    def _detect_statistical_outliers(self, container_id: str, metric_type: str,
            ...
            if value > upper_bound or value < lower_bound:
                ...
                if self._is_new_anomaly(container_id, metric_type, timestamp):
                    anomalies.append({
                        ...
                        'deviation': (value - baseline['mean']) / baseline['std_dev'] if baseline['std_dev'] > 0 else 0,
                        'type': 'outlier',
                        'severity': self._calculate_severity(value, baseline['mean'], baseline['std_dev']),
                        'description': f"Abnormal {metric_type} usage detected: {value:.2f} (baseline: {baseline['mean']:.2f})",
                    })
                    
        return anomalies
        
    def _detect_trend_anomalies(self, container_id: str, metric_type: str,
                               timestamps: List[datetime], values: List[float],
                               baseline: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
            ...
            if abs(normalized_trend) > 0.1:  # 10% change per data point
                ...
                if self._is_new_anomaly(container_id, metric_type, timestamps[-1]):
                    anomalies.append({
                        ...
                        'severity': min(3, int(abs(normalized_trend) * 10)),  # Scale 1-3
                        'description': f"Abnormal {metric_type} {'increase' if trend > 0 else 'decrease'} detected: {normalized_trend:.2%} change per data point",
                    })
                    
        return anomalies
        
    def _detect_seasonality_anomalies(self, container_id: str, metric_type: str,
                                     timestamps: List[datetime], values: List[float],
                                     baseline: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
            ...
            if latest_hour in hourly_stats:
                ...
                if hour_std_dev > 0:
                    ...
                    if abs(deviation) > self.threshold_multiplier:
                        # Check if this is a new anomaly
                        if self._is_new_anomaly(container_id, metric_type, latest_timestamp):
                            anomalies.append({
                                'container_id': container_id,
                                'metric_type': metric_type,
                                ...
                                'type': 'seasonality',
                                'hour': latest_hour,
                                'severity': self._calculate_severity(latest_value, hour_mean, hour_std_dev),
                                'description': f"Abnormal {metric_type} usage for hour {latest_hour}: {latest_value:.2f} (expected: {hour_mean:.2f})",
                            })
                            
        return anomalies
        
    def _is_new_anomaly(self, container_id: str, metric_type: str, timestamp: datetime) -> bool:
        """
        ...
        if container_id in self.last_alert_time and metric_type in self.last_alert_time[container_id]:
    ...
    def _send_anomaly_alerts(self, container_id: str, anomalies: List[Dict[str, Any]]) -> None:
        """
    ...
    def get_anomalies(self, container_id: Optional[str] = None, 
                     ...
                     severity: Optional[int] = None) -> Dict[str, List[Dict[str, Any]]]:
        """
    ...
    def _filter_anomalies(self, anomalies: List[Dict[str, Any]],
                         metric_type: Optional[str] = None,
                         start_time: Optional[datetime] = None,
                         end_time: Optional[datetime] = None,
    ...
    def get_baselines(self, container_id: Optional[str] = None, 
                     metric_type: Optional[str] = None) -> Dict[str, Dict[str, Dict[str, Any]]]:
        """
            ...
            if container_id in self.baselines:
                if metric_type:
                    if metric_type in self.baselines[container_id]:
                        return {container_id: {metric_type: self.baselines[container_id][metric_type]}}
                    else:
                        return {}
                else:
