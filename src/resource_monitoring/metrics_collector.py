...
class MetricsCollector:
    ...
    def collect_metrics(self) -> Dict[str, Dict[str, Any]]:
        ...
        for container in containers:
            try:
                ...
                logger.error(
                    "Error collecting metrics for container %s: %s",
                    container.id[:12],
                    e,
                )
    ...
    def _extract_cpu_metrics(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "throttling_periods": cpu_stats.get("throttling_data", {}).get(
                "periods", 0
            ),
            "throttled_periods": cpu_stats.get("throttling_data", {}).get(
                "throttled_periods", 0
            ),
            "throttled_time": cpu_stats.get("throttling_data", {}).get(
                "throttled_time", 0
            ),
        }
    ...
    def _extract_memory_metrics(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        memory_usage_actual = (
            memory_usage - cache if memory_usage > cache else memory_usage
        )
    ...
    def _extract_custom_metrics(
        self, container: Container, stats: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for metric in self.custom_metrics:
            try:
                ...
                if metric_type == "command":
                    ...
                    if command:
                        ...
                        if result.exit_code == 0:
                            custom_metrics[metric_name] = result.output.decode(
                                "utf-8"
                            ).strip()
                ...
                logger.error(
                    "Error collecting custom metric %s: %s", metric.get("name"), e
                )
            ...
            if container_id:
                ...
                if metric_type:
                    ...
                    if start_time or end_time:
                        ...
                        return {
                            container_id: {
                                metric_type: self.metrics_data[container_id][
                                    metric_type
                                ]
                            }
                        }
                    ...
                    if start_time or end_time:
                        ...
                        for m_type, m_data in self.metrics_data[container_id].items():
                            filtered_data[m_type] = self._filter_metrics_by_time(
                                m_data, start_time, end_time
                            )
                ...
                if metric_type:
                    ...
                    for c_id, c_data in self.metrics_data.items():
                        if metric_type in c_data:
                            if start_time or end_time:
                                result[c_id] = {
                                    metric_type: self._filter_metrics_by_time(
                                        c_data[metric_type], start_time, end_time
                                    )
                                }
                            else:
        ...
        with self.metrics_lock:
            for container_id in list(self.metrics_data.keys()):
                for metric_type in list(self.metrics_data[container_id].keys()):
                    self.metrics_data[container_id][metric_type] = [
                        m
                        for m in self.metrics_data[container_id][metric_type]
    ...
    def get_container_metrics_summary(self, container_id: str) -> Dict[str, Any]:
        ...
        cpu_avg = self._calculate_metric_average(
            container_metrics.get("cpu", []), "usage_percent"
        )

        # Calculate memory average
        memory_avg = self._calculate_metric_average(
            container_metrics.get("memory", []), "usage_percent"
        )

        # Calculate disk I/O rates
        disk_read_rate = self._calculate_metric_rate(
            container_metrics.get("disk", []), "read_bytes"
        )
        disk_write_rate = self._calculate_metric_rate(
            container_metrics.get("disk", []), "write_bytes"
        )

        # Calculate network rates
        network_rx_rate = self._calculate_metric_rate(
            container_metrics.get("network", []), "rx_bytes"
        )
        network_tx_rate = self._calculate_metric_rate(
            container_metrics.get("network", []), "tx_bytes"
        )
        ...
        return {
            ...
            "cpu": {
                ...
                "current": self._get_latest_metric_value(
                    container_metrics.get("cpu", []), "usage_percent"
                ),
                "min": self._get_min_metric_value(
                    container_metrics.get("cpu", []), "usage_percent"
                ),
                "max": self._get_max_metric_value(
                    container_metrics.get("cpu", []), "usage_percent"
                ),
            },
            "memory": {
            ...
            "disk": {
                ...
                "total_read_bytes": self._get_latest_metric_value(
                    container_metrics.get("disk", []), "read_bytes"
                ),
                "total_write_bytes": self._get_latest_metric_value(
                    container_metrics.get("disk", []), "write_bytes"
                ),
            },
            "network": {
                ...
                "total_rx_bytes": self._get_latest_metric_value(
                    container_metrics.get("network", []), "rx_bytes"
                ),
                "total_tx_bytes": self._get_latest_metric_value(
                    container_metrics.get("network", []), "tx_bytes"
                ),
            },
        }
    ...
    def _calculate_metric_rate(
        ...
        if (
            "data" not in first
            or key not in first["data"]
            or "data" not in last
            or key not in last["data"]
        ):
            return None
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            system_metrics["containers"]["running"] = len(
                [c for c in containers if c.status == "running"]
            )
            system_metrics["containers"]["paused"] = len(
                [c for c in containers if c.status == "paused"]
            )
            system_metrics["containers"]["stopped"] = len(
                ...
                try:
                    ...
                    if "disk" in summary:
                        ...
                        if disk_data.get("read_rate_bytes_per_sec") is not None:
                            total_disk_read_rate += disk_data.get(
                                "read_rate_bytes_per_sec", 0
                            )
                        if disk_data.get("write_rate_bytes_per_sec") is not None:
                            total_disk_write_rate += disk_data.get(
                                "write_rate_bytes_per_sec", 0
                            )
                    ...
                    if "network" in summary:
                        ...
                        if network_data.get("rx_rate_bytes_per_sec") is not None:
                            total_network_rx_rate += network_data.get(
                                "rx_rate_bytes_per_sec", 0
                            )
                        if network_data.get("tx_rate_bytes_per_sec") is not None:
                            total_network_tx_rate += network_data.get(
                                "tx_rate_bytes_per_sec", 0
                            )
                except Exception as e:
                    logger.error(
                        "Error aggregating metrics for container %s: %s",
                        container.id[:12],
                        e,
                    )
            ...
            if total_memory_limit > 0:
                system_metrics["memory"]["usage_percent"] = (
                    total_memory_usage / total_memory_limit
                ) * 100
    ...
    def get_metrics_history(
        ...
        if (
            not metrics
            or container_id not in metrics
            or metric_type not in metrics[container_id]
        ):
            return []
