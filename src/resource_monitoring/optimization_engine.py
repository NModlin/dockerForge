...
class OptimizationEngine:
    ...
    def __init__(
        ...
        self.optimization_config = self.config.get("resource_monitoring", {}).get(
            "optimization", {}
        )
        self.analysis_interval = self.optimization_config.get(
            "analysis_interval", 3600
        )  # seconds
        self.lookback_period = self.optimization_config.get(
            "lookback_period", 168
        )  # hours (1 week)
        self.cpu_threshold_high = self.optimization_config.get(
            "cpu_threshold_high", 80
        )  # percentage
        self.cpu_threshold_low = self.optimization_config.get(
            "cpu_threshold_low", 20
        )  # percentage
        self.memory_threshold_high = self.optimization_config.get(
            "memory_threshold_high", 80
        )  # percentage
        self.memory_threshold_low = self.optimization_config.get(
            "memory_threshold_low", 20
        )  # percentage
        self.disk_io_threshold = self.optimization_config.get(
            "disk_io_threshold", 100000000
        )  # bytes per second
        self.network_io_threshold = self.optimization_config.get(
            "network_io_threshold", 100000000
        )  # bytes per second
    ...
    def _generate_sizing_recommendations(
        self, container_id: str, summary: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if container_id:
            ...
            for c_id, recs in self.recommendations.items():
                filtered = self._filter_recommendations(
                    recs, recommendation_type, resource, start_time, end_time
                )
