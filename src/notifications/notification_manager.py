...
class NotificationManager:
            ...
            try:
                with open(history_file, "r") as f:
                    ...
                    self._notification_counts = {
                        ...
                        "last_hour_reset": datetime.fromisoformat(data["counts"]["last_hour_reset"]),
                        "last_day_reset": datetime.fromisoformat(data["counts"]["last_day_reset"]),
                    }
                    
                    logger.info(f"Loaded {len(self._notification_history)} notifications from history")
            ...
            with open(history_file, "w") as f:
                data = {
                    ...
                    "counts": {
                        ...
                        "last_hour_reset": self._notification_counts["last_hour_reset"].isoformat(),
                        "last_day_reset": self._notification_counts["last_day_reset"].isoformat(),
                    },
                }
                json.dump(data, f, indent=2)
                
                logger.debug(f"Saved {len(self._notification_history)} notifications to history")
        except Exception as e:
    ...
    def _should_throttle(self, notification: Notification) -> bool:
        ...
        throttling_enabled = get_config("notifications.preferences.throttling.enabled", True)
        
        if not throttling_enabled:
            return False
        
        # Check quiet hours
        quiet_hours_enabled = get_config("notifications.preferences.throttling.quiet_hours.enabled", False)
        
        if quiet_hours_enabled:
            now = datetime.now()
            start_time_str = get_config("notifications.preferences.throttling.quiet_hours.start", "22:00")
            end_time_str = get_config("notifications.preferences.throttling.quiet_hours.end", "08:00")
            ...
            if start_time > end_time:
                ...
                if now >= start_time or now <= end_time:
                    ...
                    if notification.severity != NotificationSeverity.CRITICAL:
                        logger.debug(f"Throttling notification during quiet hours: {notification.title}")
                        return True
            else:
        ...
        # Check hourly limit
        max_hourly = get_config("notifications.preferences.throttling.max_notifications_per_hour", 10)
        
        if self._notification_counts["hourly"] >= max_hourly:
            # Only throttle non-critical notifications when hourly limit is reached
            if notification.severity != NotificationSeverity.CRITICAL:
                logger.debug(f"Throttling notification due to hourly limit: {notification.title}")
                return True
        
        # Check daily limit
        max_daily = get_config("notifications.preferences.throttling.max_notifications_per_day", 50)
        
        if self._notification_counts["daily"] >= max_daily:
            # Only throttle non-critical notifications when daily limit is reached
            if notification.severity != NotificationSeverity.CRITICAL:
                logger.debug(f"Throttling notification due to daily limit: {notification.title}")
                return True
        
        # Check for similar notifications
        group_similar = get_config("notifications.preferences.throttling.group_similar", True)
        ...
        if group_similar:
            ...
            for n in self._notification_history:
                if n.created_at >= one_hour_ago and n.title == notification.title:
                    ...
                    logger.debug(f"Throttling similar notification: {notification.title}")
                    return True
    ...
    def _should_send_by_severity(self, severity: NotificationSeverity) -> bool:
        ...
        thresholds = {
            NotificationSeverity.INFO: get_config("notifications.preferences.severity_thresholds.info", False),
            NotificationSeverity.WARNING: get_config("notifications.preferences.severity_thresholds.warning", True),
            NotificationSeverity.ERROR: get_config("notifications.preferences.severity_thresholds.error", True),
            NotificationSeverity.CRITICAL: get_config("notifications.preferences.severity_thresholds.critical", True),
        }
    ...
    def _should_send_by_type(self, notification_type: NotificationType) -> bool:
        ...
        type_prefs = {
            NotificationType.CONTAINER_EXIT: get_config("notifications.preferences.notification_types.container_exit", True),
            NotificationType.CONTAINER_OOM: get_config("notifications.preferences.notification_types.container_oom", True),
            NotificationType.HIGH_RESOURCE_USAGE: get_config("notifications.preferences.notification_types.high_resource_usage", True),
            NotificationType.SECURITY_ISSUE: get_config("notifications.preferences.notification_types.security_issue", True),
            NotificationType.UPDATE_AVAILABLE: get_config("notifications.preferences.notification_types.update_available", True),
            NotificationType.FIX_PROPOSAL: get_config("notifications.preferences.notification_types.fix_proposal", True),
            NotificationType.FIX_APPLIED: get_config("notifications.preferences.notification_types.fix_applied", True),
    ...
    def send_notification(self, notification: Notification) -> bool:
        ...
        if not self._should_send_by_severity(notification.severity):
            logger.debug(f"Not sending notification due to severity threshold: {notification.title}")
            return False
        
        # Check if notification should be sent based on type
        if not self._should_send_by_type(notification.notification_type):
            logger.debug(f"Not sending notification due to type preference: {notification.title}")
            return False
        
        # Check if notification should be throttled
        if self._should_throttle(notification):
            logger.debug(f"Not sending notification due to throttling: {notification.title}")
            return False
        ...
        if not channels:
            logger.warning(f"No channels available to send notification: {notification.title}")
            return False
            ...
            if channel in self._notifiers:
                try:
                    ...
                    if result:
                        ...
                        success = True
                        logger.info(f"Sent notification to {channel}: {notification.title}")
                    else:
                        logger.warning(f"Failed to send notification to {channel}: {notification.title}")
                except Exception as e:
    ...
    def create_and_send_notification(
        ...
        if isinstance(notification_type, str):
            try:
                ...
                logger.warning(f"Invalid notification type: {notification_type}, using CUSTOM")
    ...
    def acknowledge_notification(self, notification_id: str, user: Optional[str] = None) -> bool:
