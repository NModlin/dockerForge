...
def setup_vulnerability_scan_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "scan", help="Scan Docker images for vulnerabilities"
    )
    parser.add_argument(
        "--image",
...
def setup_audit_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "audit", help="Audit Docker configuration for security best practices"
    )
    parser.add_argument(
        "--check-type",
...
def setup_report_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "report", help="Generate a comprehensive security report"
    )
    parser.add_argument(
        "--image",
    ...
    try:
        ...
        if args.output:
            ...
            if args.format == "json":
                ...
                src_file = report.get(
                    "html_file" if args.format == "html" else "text_file"
                )
