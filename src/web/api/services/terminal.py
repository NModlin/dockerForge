...
class TerminalManager:
    ...
    def create_session(
        self, container_id: str, user_id: str, cols: int = 80, rows: int = 24
    ) -> str:
        """
        ...
        logger.info(
            f"Created terminal session {session_id} for container {container_id} by user {user_id}"
        )
        return session_id
...
class TerminalSession:
    ...
    async def connect(self, websocket: WebSocket) -> bool:
        ...
        try:
            ...
            exec_socket = docker_client.api.exec_start(
                self.exec_id, detach=False, tty=True, socket=True
            )
            ...
            logger.info(
                f"Connected terminal session {self.session_id} to container {self.container_id}"
            )
            return True

        except DockerException as e:
            logger.error(
                f"Docker error connecting terminal session {self.session_id}: {str(e)}"
            )
            await websocket.send_json(
                {
                    "type": "error",
            ...
            logger.error(
                f"Error connecting terminal session {self.session_id}: {str(e)}"
            )
            await websocket.send_json(
                {"type": "error", "error": f"Error: {str(e)}", "timestamp": time.time()}
            )
            return False
    ...
    async def send_input(self, data: str) -> bool:
        ...
        try:
            ...
            docker_client.api.exec_resize(
                self.exec_id, height=self.rows, width=self.cols
            )
            ...
            logger.error(
                f"Error sending input to terminal session {self.session_id}: {str(e)}"
            )
            return False
    ...
    async def _handle_container_output(self, socket) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error handling container output for session {self.session_id}: {str(e)}"
            )
            if self.socket:
                try:
                    ...
                    logger.error(
                        f"Failed to send error message to socket: {socket_err}"
                    )
        finally:
            # Close the session
            self.close()
