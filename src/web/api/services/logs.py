...
async def get_logs(
    db: Session, filter_params: LogFilter = None, skip: int = 0, limit: int = 100
) -> List[LogEntry]:
    """
    ...
    if filter_params:
        ...
        if filter_params.message_contains:
            query = query.filter(
                LogEntry.message.ilike(f"%{filter_params.message_contains}%")
            )
        ...
        if filter_params.stream:
            if filter_params.stream == LogStream.BOTH:
                query = query.filter(
                    LogEntry.log_metadata["stream"].astext.in_(
                        [LogStream.STDOUT, LogStream.STDERR]
                    )
                )
            else:
                query = query.filter(
                    LogEntry.log_metadata["stream"].astext == filter_params.stream
                )
...
async def get_log_statistics(
    db: Session, filter_params: LogFilter = None, days: int = 7
) -> LogStatistics:
    """
    Get log statistics.
    ...
    level_counts = (
        ...
        .filter(LogEntry.timestamp >= start_date)
    ...
    source_counts = (
        db.query(LogEntry.source, func.count(LogEntry.id))
        .filter(LogEntry.timestamp >= start_date)
    ...
    stream_counts = (
        db.query(LogEntry.log_metadata["stream"].astext, func.count(LogEntry.id))
        .filter(
        ...
        .group_by(LogEntry.log_metadata["stream"].astext)
        .all()
    )
...
async def export_logs(
    db: Session, filter_params: LogFilter, format: LogExportFormat
) -> Tuple[str, bytes]:
    """
    ...
    if format == LogExportFormat.JSON:
        ...
        for log in logs:
            ...
            lines.append(
                f"{log.timestamp.isoformat()} {source}{stream} [{log.level.upper()}] {log.message}"
            )
...
async def get_log_aggregation_settings() -> LogAggregationSettings:
    ...
    collection_interval = get_config(
        "monitoring.log_aggregation.collection_interval", 60
    )
    ...
    max_entries = get_config(
        "monitoring.log_aggregation.retention.max_entries", 1000000
    )

    # Get sources
    sources = {}
    for source in LogSource:
        sources[source] = get_config(
            f"monitoring.log_aggregation.sources.{source}", True
        )
...
async def update_log_aggregation_settings(settings: LogAggregationSettings) -> bool:
    ...
    config["monitoring"]["log_aggregation"][
        "collection_interval"
...
async def analyze_logs(
    container_id: str, hours: int = 24, include_patterns: bool = True
) -> Dict[str, Any]:
    """
    ...
    analysis = log_explorer.analyze_container_logs(
        container_id, start_time=start_time, include_patterns=include_patterns
    )
