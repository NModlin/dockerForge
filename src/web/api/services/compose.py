...
async def get_compose_files(
    name: Optional[str] = None, db: Session = None
) -> List[ComposeFile]:
    """
...
""",
        ...
        if name:
            filtered_files = [
                f for f in filtered_files if name.lower() in f["name"].lower()
            ]
...
async def create_compose_file(
    ...
    try:
        ...
        try:
            ...
            if (
                not yaml_content
                or not isinstance(yaml_content, dict)
                or "services" not in yaml_content
            ):
    ...
    try:
        ...
        if content is not None:
            ...
            try:
                ...
                # Parse services from YAML
                services = []
                for service_name, service_config in yaml_content.get(
                    "services", {}
                ).items():
                    ...
                    existing_service = next(
                        (
                            s
                            for s in updated_file["services"]
                    ...
                    service = {
                        ...
                        "command": service_config.get("command", None),
                        "container_id": (
                            existing_service["container_id"]
                            if existing_service
                            else None
                        ),
                        "container_name": (
                            existing_service["container_name"]
                            if existing_service
                            else f"{updated_file['name'].lower().replace(' ', '_')}_{service_name}_1"
                        ),
                    }
                    services.append(service)
...
async def get_compose_logs(
    ...
    try:
        ...
        if service:
            ...
            service_obj = next(
                (s for s in compose_file.services if s.name == service), None
            )
            if not service_obj:
                raise ValueError(
                    f"Service {service} not found in compose file {compose_file.name}"
                )

            logs[service] = [
                f"[{service}] {datetime.now().isoformat()} Log line {i+1}"
                for i in range(tail)
            ]
        else:
            ...
            for service_obj in compose_file.services:
                logs[service_obj.name] = [
                    f"[{service_obj.name}] {datetime.now().isoformat()} Log line {i+1}"
                    for i in range(tail)
                ]
