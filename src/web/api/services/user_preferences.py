...
from src.web.api.schemas.user_preferences import UserPreferencesCreate, UserPreferencesUpdate
...
def create_user_preferences(db: Session, preferences_data: UserPreferencesCreate, user_id: int) -> UserPreferences:
    """
    ...
    preferences = UserPreferences(
        ...
        notification_preferences=preferences_data.notifications.dict() if preferences_data.notifications else None,
        display_preferences=preferences_data.display.dict() if preferences_data.display else None,
        keyboard_preferences=preferences_data.keyboard.dict() if preferences_data.keyboard else None
    )
...
def update_user_preferences(db: Session, preferences_data: UserPreferencesUpdate, user_id: int) -> UserPreferences:
    """
    ...
    if preferences_data.notifications:
        ...
        preferences.notification_preferences.update(preferences_data.notifications.dict(exclude_unset=True))
    ...
    if preferences_data.display:
        ...
        # Update with new values
        preferences.display_preferences.update(preferences_data.display.dict(exclude_unset=True))
    ...
    if preferences_data.keyboard:
        ...
        # Update with new values
        preferences.keyboard_preferences.update(preferences_data.keyboard.dict(exclude_unset=True))
