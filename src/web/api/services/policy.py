...
async def update_policy(
    policy_id: str, policy_update: PolicyUpdate
...
async def evaluate_policy(
    ...
    if policy.applied_to and not any(
        re.match(pattern, resource_name) for pattern in policy.applied_to
    ):
        return result
    ...
    for rule in policy.rules:
        ...
        if rule.rule_type == PolicyRuleType.SEVERITY and scan_result:
            ...
            rule.rule_type == PolicyRuleType.CONTAINER and resource_type == "container"
        ):
            ...
            rule.rule_type == PolicyRuleType.PRIVILEGED and resource_type == "container"
        ):
            ...
            try:
                ...
                logger.error(
                    f"Error checking privileged status for container {resource_id}: {str(e)}"
                )
...
async def evaluate_all_policies(
    resource_type: str, resource_id: str, scan_result: Optional[ScanResult] = None
    ...
    for policy_id in policies:
        result = await evaluate_policy(
            policy_id, resource_type, resource_id, scan_result
        )
...
async def get_compliance_status() -> ComplianceStatus:
    ...
    for violation in policy_violations:
        ...
        if (
            resource_key not in evaluated_resources
            and violation.action_taken == PolicyAction.BLOCK
    ...
    for violation in policy_violations:
        if violation.action_taken == PolicyAction.BLOCK and not violation.resolved:
            policy_violation_counts[violation.policy_id] = (
                policy_violation_counts.get(violation.policy_id, 0) + 1
            )
...
async def get_violations(
    ...
    if policy_id:
        filtered_violations = [
            v for v in filtered_violations if v.policy_id == policy_id
        ]

    if resource_type:
        filtered_violations = [
            v for v in filtered_violations if v.resource_type == resource_type
        ]

    if resource_id:
        filtered_violations = [
            v for v in filtered_violations if v.resource_id == resource_id
        ]
...
async def resolve_violation(
    violation_id: str, resolution_notes: Optional[str] = None
