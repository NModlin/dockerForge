...
class ComposeProject(BaseModel, TimestampMixin):
    ...
    status = Column(String(20), nullable=False, default='inactive')  # inactive, active, error
    ...
    # Relationships
    files = relationship("ComposeFile", back_populates="project", cascade="all, delete-orphan")
...
class ComposeFile(BaseModel, TimestampMixin):
    ...
    name = Column(String(100), nullable=False)  # e.g., docker-compose.yml, docker-compose.override.yml
    content = Column(Text, nullable=False)
    is_main = Column(Boolean, nullable=False, default=False)  # Is this the main compose file?
...
class ComposeService(BaseModel, TimestampMixin):
    ...
    containers = relationship("ComposeServiceContainer", back_populates="service", cascade="all, delete-orphan")
