...
"""
from sqlalchemy import Column, Integer, String, Text, DateTime, ForeignKey, Boolean, JSON, Table, Float
...
class ApiKeyUsage(BaseModel, TimestampMixin):
    ...
    endpoint = Column(String(255), nullable=False, index=True)  # API endpoint that was accessed
...
class ApiKeyUsageSummary(BaseModel):
    ...
    total_requests = Column(Integer, nullable=False, default=0)  # Total number of requests
    successful_requests = Column(Integer, nullable=False, default=0)  # Number of successful requests (2xx)
    failed_requests = Column(Integer, nullable=False, default=0)  # Number of failed requests (4xx, 5xx)
    avg_response_time = Column(Float, nullable=False, default=0)  # Average response time in milliseconds
    total_request_size = Column(Integer, nullable=False, default=0)  # Total size of requests in bytes
    total_response_size = Column(Integer, nullable=False, default=0)  # Total size of responses in bytes
    endpoints = Column(JSON, nullable=True)  # Dictionary of endpoints and request counts
