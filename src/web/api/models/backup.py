...
class Backup(BaseModel, TimestampMixin):
    ...
    )  # container, image, volume, compose, system
    status = Column(
    ...
    # Relationships
    backup_items = relationship(
        "BackupItem", back_populates="backup", cascade="all, delete-orphan"
    )
...
class BackupItem(BaseModel, TimestampMixin):
    ...
    )  # ID of the item (container ID, image ID, etc.)
...
class RestoreJob(BaseModel, TimestampMixin):
    ...
    restore_items = relationship(
        "RestoreItem", back_populates="restore_job", cascade="all, delete-orphan"
    )
