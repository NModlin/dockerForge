...
class ChatMessage(Base):
    ...
    context = Column(JSON, nullable=True)  # Store context information (page, container ID, etc.)
...
class UserPreference(Base):
    ...
    response_style = Column(String(20), default="balanced", nullable=False)  # 'technical', 'simple', 'balanced'
    auto_suggestions = Column(Boolean, default=True, nullable=False)
    preferred_topics = Column(JSON, nullable=True)  # List of topics the user frequently discusses
    avoided_topics = Column(JSON, nullable=True)  # Topics the user prefers to avoid
    feedback_preferences = Column(JSON, nullable=True)  # Feedback patterns stored for reference
    created_at = Column(DateTime, default=datetime.datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow, nullable=False)
...
class ChatCommandShortcut(Base):
    ...
    description = Column(String(255), nullable=False)  # Description of what the command does
...
class ConversationMemory(Base):
    ...
    key_information = Column(JSON, nullable=True)  # Extracted key information from the message
    importance_score = Column(Float, default=1.0, nullable=False)  # Higher score for more important information
    created_at = Column(DateTime, default=datetime.datetime.utcnow, nullable=False)
...
ChatMessage.feedback_entries = relationship("ChatFeedback", back_populates="message", cascade="all, delete-orphan")
