...
"""
from sqlalchemy import Column, String, DateTime, Boolean, ForeignKey, JSON, Integer, Text, Float
...
class MetricSample(BaseModel, TimestampMixin):
    ...
    metric_type = Column(String(50), nullable=False, index=True)  # cpu, memory, network, disk, etc.
...
class Alert(BaseModel, TimestampMixin):
    ...
    source = Column(String(50), nullable=False)  # container, image, volume, network, system
    source_id = Column(String(100), nullable=True)  # ID of the source (container ID, etc.)
...
class AlertRule(BaseModel, TimestampMixin):
    ...
    source_type = Column(String(50), nullable=False)  # container, image, volume, network, system
    source_filter = Column(JSON, nullable=True)  # Filter for source (e.g., container name pattern)
    metric_type = Column(String(50), nullable=False)  # cpu, memory, network, disk, etc.
    condition = Column(String(20), nullable=False)  # >, <, >=, <=, ==, !=
    threshold = Column(Float, nullable=False)
    duration = Column(Integer, nullable=True)  # Duration in seconds for condition to be true
...
class LogEntry(BaseModel, TimestampMixin):
    ...
    source = Column(String(50), nullable=False, index=True)  # container, image, volume, network, system
    source_id = Column(String(100), nullable=True, index=True)  # ID of the source (container ID, etc.)
