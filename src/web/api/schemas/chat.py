...
class ContextData(BaseModel):
    ...
    current_container_id: Optional[str] = Field(None, description="Current container ID")
...
class ChatResponse(BaseModel):
    ...
    session_id: int = Field(..., description="Chat session ID")
    suggestions: Optional[List[str]] = Field(None, description="Suggested user responses")
...
class UserPreferenceBase(BaseModel):
    ...
    """
    response_style: Literal["technical", "simple", "balanced"] = Field("balanced", description="Preferred response style")
    auto_suggestions: bool = Field(True, description="Whether to show auto-suggestions")
    preferred_topics: Optional[List[str]] = Field(None, description="Topics the user frequently discusses")
    avoided_topics: Optional[List[str]] = Field(None, description="Topics the user prefers to avoid")
...
class UserPreferenceUpdate(BaseModel):
    ...
    """
    response_style: Optional[Literal["technical", "simple", "balanced"]] = Field(None, description="Preferred response style")
    auto_suggestions: Optional[bool] = Field(None, description="Whether to show auto-suggestions")
    preferred_topics: Optional[List[str]] = Field(None, description="Topics the user frequently discusses")
    avoided_topics: Optional[List[str]] = Field(None, description="Topics the user prefers to avoid")
...
class UserPreference(UserPreferenceBase):
    ...
    feedback_preferences: Optional[Dict[str, Any]] = Field(None, description="Feedback patterns stored for reference")
    created_at: datetime = Field(..., description="Creation timestamp")
    updated_at: datetime = Field(..., description="Last update timestamp")
...
class ChatCommandShortcutBase(BaseModel):
    ...
    """
    command: str = Field(..., description="Shortcut command (e.g., /logs)", min_length=2, max_length=50)
    description: str = Field(..., description="Description of what the command does", max_length=255)
...
class ChatCommandShortcutUpdate(BaseModel):
    ...
    """
    description: Optional[str] = Field(None, description="Description of what the command does", max_length=255)
    template: Optional[str] = Field(None, description="Command template with placeholders")
...
class ChatCommandShortcut(ChatCommandShortcutBase):
    ...
    user_id: int = Field(..., description="User ID")
    usage_count: int = Field(..., description="Number of times this shortcut has been used")
    created_at: datetime = Field(..., description="Creation timestamp")
    updated_at: datetime = Field(..., description="Last update timestamp")
...
class CommandShortcutsList(BaseModel):
    ...
    """
    shortcuts: List[ChatCommandShortcut] = Field(..., description="List of command shortcuts")
...
class EnhancedChatResponse(ChatResponse):
    ...
    """
    feedback_id: Optional[int] = Field(None, description="ID of feedback if already provided")
    command_shortcuts: Optional[List[ChatCommandShortcut]] = Field(None, description="Available command shortcuts")
    user_preferences: Optional[UserPreference] = Field(None, description="User preferences")
