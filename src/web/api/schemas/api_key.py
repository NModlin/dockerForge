...
class ApiKeyBase(BaseModel):
    ...
    scopes: Optional[List[str]] = Field(
        None, description="Permission scopes for the API key"
    )
...
class ApiKeyCreate(ApiKeyBase):
    ...
    expiration: Optional[str] = Field(
        None, description="Expiration period (e.g., '30d', '90d', '1y', or 'never')"
    )
    ...
    def validate_expiration(cls, v):
        if v is not None and v != "never" and not (v.endswith("d") or v.endswith("y")):
            raise ValueError(
                "Expiration must be 'never' or end with 'd' (days) or 'y' (years)"
            )
        return v
...
class ApiKeyUpdate(BaseModel):
    ...
    is_read_only: Optional[bool] = Field(
        None, description="Whether the API key is read-only"
    )
    scopes: Optional[List[str]] = Field(
        None, description="Permission scopes for the API key"
    )
