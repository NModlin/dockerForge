...
class RegistryConfig(BaseModel):
    ...
    registry_mirrors: List[str] = Field(
        default_factory=list, description="Registry mirrors", alias="registry-mirrors"
    )
...
class LoggingConfig(BaseModel):
    ...
    log_driver: str = Field(
        default="json-file", description="Logging driver", alias="log-driver"
    )
    log_opts: Dict[str, str] = Field(
        default_factory=dict, description="Logging driver options", alias="log-opts"
    )
...
class StorageConfig(BaseModel):
    ...
    storage_driver: Optional[str] = Field(
        default=None, description="Storage driver", alias="storage-driver"
    )
    storage_opts: List[str] = Field(
        default_factory=list, description="Storage driver options", alias="storage-opts"
    )
    data_root: Optional[str] = Field(
        default=None, description="Docker data root directory", alias="data-root"
    )
...
class NetworkConfig(BaseModel):
    ...
    dns_opts: List[str] = Field(
        default_factory=list, description="DNS options", alias="dns-opts"
    )
    dns_search: List[str] = Field(
        default_factory=list, description="DNS search domains", alias="dns-search"
    )
    bip: Optional[str] = Field(default=None, description="Docker bridge IP")
    fixed_cidr: Optional[str] = Field(
        default=None, description="Fixed CIDR", alias="fixed-cidr"
    )
    fixed_cidr_v6: Optional[str] = Field(
        default=None, description="Fixed CIDR for IPv6", alias="fixed-cidr-v6"
    )
    default_gateway: Optional[str] = Field(
        default=None, description="Default gateway", alias="default-gateway"
    )
    default_gateway_v6: Optional[str] = Field(
        default=None, description="Default gateway for IPv6", alias="default-gateway-v6"
    )
    ip_forward: Optional[bool] = Field(
        default=None, description="IP forwarding", alias="ip-forward"
    )
    ip_masq: Optional[bool] = Field(
        default=None, description="IP masquerading", alias="ip-masq"
    )
    iptables: Optional[bool] = Field(
        default=None, description="Enable iptables", alias="iptables"
    )
    ipv6: Optional[bool] = Field(default=None, description="Enable IPv6")
...
class AvailableDrivers(BaseModel):
    ...
    logging_drivers: List[DriverInfo] = Field(
        ..., description="Available logging drivers"
    )
    storage_drivers: List[DriverInfo] = Field(
        ..., description="Available storage drivers"
    )
...
class ConfigUpdateResponse(BaseModel):
    ...
    requires_restart: bool = Field(
        default=False, description="Whether a restart is required"
    )
