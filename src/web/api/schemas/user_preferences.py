...
class NotificationPreferences(BaseModel):
    ...
    events: List[str] = Field(
        default_factory=lambda: ["error", "security"], description="Events to notify"
    )
...
class DisplayPreferences(BaseModel):
    ...
    showSystemContainers: bool = Field(
        False, description="Whether to show system containers"
    )
...
class KeyboardPreferences(BaseModel):
    ...
    enableShortcuts: bool = Field(
        True, description="Whether keyboard shortcuts are enabled"
    )
    customShortcuts: Dict[str, str] = Field(
        default_factory=dict, description="Custom keyboard shortcuts"
    )
...
class UserPreferencesBase(BaseModel):
    ...
    notifications: Optional[NotificationPreferences] = Field(
        None, description="Notification preferences"
    )
    display: Optional[DisplayPreferences] = Field(
        None, description="Display preferences"
    )
    keyboard: Optional[KeyboardPreferences] = Field(
        None, description="Keyboard preferences"
    )
