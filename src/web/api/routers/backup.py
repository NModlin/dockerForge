...
from src.web.api.models.backup import Backup, BackupItem, RestoreItem, RestoreJob
...
async def get_backups(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """
    Get all backups.
    """
...
async def create_backup(backup: schemas.BackupCreate, db: Session = Depends(get_db)):
    """
    Create a new backup.
    """
...
async def update_backup(
    backup_id: int, backup: schemas.BackupUpdate, db: Session = Depends(get_db)
):
    """
    Update a backup.
    """
...
async def get_backup_items(
    backup_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    """
    ...
    items = (
        db.query(BackupItem)
        .filter(BackupItem.backup_id == backup_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.post(
    "/{backup_id}/items",
    response_model=schemas.BackupItem,
    status_code=status.HTTP_201_CREATED,
)
async def create_backup_item(
    backup_id: int, item: schemas.BackupItemCreate, db: Session = Depends(get_db)
):
    """
...
@router.post(
    "/restore", response_model=schemas.RestoreJob, status_code=status.HTTP_201_CREATED
)
async def create_restore_job(
    job: schemas.RestoreJobCreate, db: Session = Depends(get_db)
):
    """
...
async def get_restore_job(job_id: int, db: Session = Depends(get_db)):
    """
...
async def update_restore_job(
    job_id: int, job: schemas.RestoreJobUpdate, db: Session = Depends(get_db)
):
    """
...
async def delete_restore_job(job_id: int, db: Session = Depends(get_db)):
    """
...
@router.post(
    "/restore/{job_id}/items",
    response_model=schemas.RestoreItem,
    status_code=status.HTTP_201_CREATED,
)
async def create_restore_item(
    job_id: int, item: schemas.RestoreItemCreate, db: Session = Depends(get_db)
):
    """
    ...
    db_backup_item = (
        db.query(BackupItem).filter(BackupItem.id == item.backup_item_id).first()
    )
