...
from src.core.ai_provider import AIProviderError, AIProviderFactory, get_ai_provider
...
from src.web.api.services import alerts, host_stats
...
async def get_host_metrics_history(
    ...
    hours: int = Query(
        1, ge=1, le=24, description="Number of hours of history to retrieve"
    ),
):
    """
    ...
    try:
        ...
        # Get metrics history
        metrics = await host_stats.get_host_metrics_history(
    ...
    try:
        ...
        for name, info in providers_info.items():
            ...
            providers_status[name] = schemas.AIProviderStatus(
                ...
                capabilities=(
                    schemas.AICapabilities(**capabilities) if capabilities else None
                ),
            )

        return schemas.AIStatusResponse(
            providers=providers_status, default_provider=default_provider
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.post(
    "/troubleshoot/container/{container_id}",
    response_model=schemas.ContainerTroubleshootingResult,
)
async def troubleshoot_container(
    container_id: str, request: schemas.ContainerTroubleshootingRequest
):
    """
    ...
    try:
        ...
        result = troubleshooter.analyze_container(
            container_id=container_id, confirm_cost=request.confirm_cost
        )
...
async def troubleshoot_logs(request: schemas.LogsTroubleshootingRequest):
    """
...
async def troubleshoot_compose(request: schemas.DockerComposeRequest):
    """
...
async def troubleshoot_dockerfile(request: schemas.DockerfileRequest):
    """
...
@router.get(
    "/troubleshoot/connection", response_model=schemas.ConnectionTroubleshootingResult
)
