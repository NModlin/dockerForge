...
from src.web.api.schemas.logs import (
    LogEntry, LogEntryCreate, LogFilter, LogExportFormat,
    LogStatistics, LogAggregationSettings, LogLevel, LogSource, LogStream
)
...
async def get_logs(
    ...
    message_contains: Optional[str] = Query(None, description="Filter by message content"),
    ...
    stream: Optional[str] = Query(None, description="Filter by stream (for containers)"),
...
async def get_container_logs(
    ...
    tail: int = Query(100, description="Number of lines to return from the end of the logs"),
    since: Optional[str] = Query(None, description="Only return logs since this timestamp (ISO format)"),
    until: Optional[str] = Query(None, description="Only return logs before this timestamp (ISO format)"),
    stream: Optional[str] = Query(None, description="Stream to filter by (stdout, stderr, or both)"),
    follow: bool = Query(False, description="Whether to follow the logs (stream updates)"),
    current_user: User = Depends(get_current_active_user)
):
    """
...
async def get_multi_container_logs(
    ...
    until: Optional[str] = Query(None, description="Only return logs before this timestamp (ISO format)"),
    stream: Optional[str] = Query(None, description="Stream to filter by (stdout, stderr, or both)"),
    message_contains: Optional[str] = Query(None, description="Only return logs containing this string"),
    message_regex: Optional[str] = Query(None, description="Only return logs matching this regex"),
    current_user: User = Depends(get_current_active_user)
):
    """
...
async def analyze_logs(
    ...
    include_patterns: bool = Query(True, description="Whether to include pattern matching"),
    current_user: User = Depends(get_current_active_user)
):
    """
