...
from src.web.api.schemas.api_key import (
    ApiKeyCreate, ApiKeyResponse, ApiKeyDetail, ApiKeyList,
    ApiKeyUsageResponse, ApiKeyUsageStats, ApiKeyUsageRecord
)
...
async def get_api_key_usage(
    ...
    try:
        ...
        ).order_by(ApiKeyUsage.created_at.desc()).offset(skip).limit(limit).all()
        
        # Count total records
        total_records = db.query(ApiKeyUsage).filter(
            ApiKeyUsage.api_key_id == key_id
        ).count()
