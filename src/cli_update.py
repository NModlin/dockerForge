...
def check(ctx, force):
    ...
    update_available, latest_version, release_url = version_checker.check_for_updates(force=force)
...
@click.option("--force", is_flag=True, help="Force update even if already at latest version")
...
@update_cli.command()
@click.option("--backup-id", help="Specific backup ID to rollback to (default: most recent)")
@click.option("--yes", "-y", is_flag=True, help="Skip confirmation prompt")
@click.pass_context
