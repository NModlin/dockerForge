...
from src.config.config_manager import get_config, save_config, set_config
...
@click.option(
    "--channel",
    "-ch",
    type=click.Choice(["email", "slack", "discord", "webhook", "all"]),
    default="all",
    help="Notification channel",
)
def send_notification(
    title, message, severity, notification_type, container, issue, fix, channel
):
    """Send a notification."""
...
@click.option(
    "--acknowledged", "-a", is_flag=True, help="Show only acknowledged notifications"
)
@click.option(
    "--unacknowledged",
    "-u",
...
def list_notifications(
    ...
    try:
        ...
        if export:
            ...
            click.secho(
                f"Exported {len(notifications)} notifications to {export}", fg="green"
            )
            return
        ...
        for notification in notifications:
            ...
            if notification.acknowledged:
                click.echo(
                    f"Acknowledged: Yes, by {notification.acknowledged_by or 'unknown'} at {notification.acknowledged_at.isoformat()}"
                )
            else:
...
def acknowledge_notification(notification_id, user):
    ...
    try:
        ...
        if result:
            ...
            click.secho(
                f"Failed to acknowledge notification: {notification_id}", fg="red"
            )
            sys.exit(1)
    except Exception as e:
...
@click.option(
    ...
    "-s",
    multiple=True,
    help="Add a step (format: title|description|command|code|file_path|manual_action|verification)",
)
    ...
    try:
        ...
        for s in step:
            ...
            if len(parts) < 2:
                ...
                click.echo(
                    "Format: title|description|command|code|file_path|manual_action|verification"
                )
                sys.exit(1)
...
@click.option(
    ...
    "-s",
    type=click.Choice(
        ["proposed", "approved", "rejected", "applied", "failed", "rolled_back"]
    ),
    ...
    try:
        ...
        for fix in fixes:
            ...
            if fix.status == FixStatus.APPROVED:
                click.echo(
                    f"Approved: {fix.approved_at.isoformat()} by {fix.approved_by or 'unknown'}"
                )

            if fix.status == FixStatus.REJECTED:
                click.echo(
                    f"Rejected: {fix.rejected_at.isoformat()} by {fix.rejected_by or 'unknown'}"
                )
...
def show_fix(fix_id):
    ...
    try:
        ...
        if (
            fix.status in [FixStatus.APPLIED, FixStatus.FAILED, FixStatus.ROLLED_BACK]
            and fix.result
        ):
            ...
            if fix.result.get("steps"):
                ...
                for i, step_result in enumerate(fix.result["steps"]):
                    ...
                    click.secho(
                        f"    {i+1}. {step_title}: {'Success' if step_success else 'Failed'}",
                        fg=step_color,
                    )
                    ...
                    if step_result.get("verification"):
                        ...
                        click.secho(
                            f"       Verification: {'Success' if verification_success else 'Failed'}",
    ...
    try:
        ...
        if result["success"]:
            ...
            if result.get("partial"):
                click.secho(
                    f"Fix partially rolled back with errors: {fix_id}", fg="yellow"
                )
            else:
