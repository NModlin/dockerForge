...
def setup_checkpoint_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "checkpoint",
        help="Create, compare, and restore Docker state via checkpoints"
    )
    ...
    show_parser.add_argument(
        "checkpoint_id",
        help="ID of the checkpoint to show"
    )
    show_parser.add_argument(
        "--json",
        action="store_true",
    ...
    compare_parser.add_argument(
        "checkpoint_id_1",
        help="ID of the first checkpoint"
    )
    compare_parser.add_argument(
        "checkpoint_id_2",
        help="ID of the second checkpoint"
    )
    compare_parser.add_argument(
        "--no-containers",
        action="store_true",
        help="Don't compare containers"
    )
    compare_parser.add_argument(
        "--no-volumes",
        action="store_true",
        help="Don't compare volumes"
    )
    compare_parser.add_argument(
        "--no-configs",
        action="store_true",
        help="Don't compare configuration files"
    )
    compare_parser.add_argument(
        "--json",
        action="store_true",
    ...
    restore_parser.add_argument(
        "checkpoint_id",
        help="ID of the checkpoint to restore from"
    )
    restore_parser.add_argument(
        "--no-containers",
        action="store_true",
        help="Don't restore containers"
    )
    restore_parser.add_argument(
        "--with-volumes",
        action="store_true",
        help="Restore volumes (use with caution)"
    )
    restore_parser.add_argument(
        "--no-configs",
        action="store_true",
        help="Don't restore configuration files"
    )
...
def handle_list_checkpoints(args):
    ...
    try:
        ...
        # Print table header
        print(f"{'ID':<40} {'Name':<20} {'Description':<30} {'Timestamp':<25} {'Items'}")
        print("-" * 120)
        ...
        for cp in checkpoints:
            ...
            print(f"{checkpoint_id:<40} {name:<20} {description:<30} {timestamp_str:<25} {items_str}")
...
def handle_show_checkpoint(args):
    ...
    try:
        ...
        print("\nIncludes:")
        print(f"  Containers: {'Yes' if checkpoint.get('include_containers') else 'No'}")
...
def handle_delete_checkpoint(args):
    ...
    try:
        ...
        # Confirm deletion
        confirm = input(f"Are you sure you want to delete checkpoint '{checkpoint_name}' ({args.checkpoint_id})? [y/N] ")
                ...
                for container in container_comparison.get("modified", []):
                    ...
                    if details.get("modified_values"):
                        for key, value in details.get("modified_values", {}).items():
                            print(f"      {key}: {value.get('old')} -> {value.get('new')}")
            ...
            if config_comparison.get("modified"):
                ...
                if config_comparison.get("diffs"):
                    ...
                    if show_diffs.lower() in ["y", "yes"]:
                        for file_name, diff in config_comparison.get("diffs", {}).items():
                            print(f"\nDiff for {file_name}:")
                            print(diff)
...
def handle_restore_checkpoint(args):
    ...
    try:
        ...
        # Confirm restoration
        print(f"You are about to restore from checkpoint '{checkpoint_name}' ({args.checkpoint_id}).")
        print(f"This will potentially stop and recreate containers and modify configuration files.")
        if args.with_volumes:
            print(f"WARNING: Volume data will also be restored. This is a destructive operation.")
        ...
        if result.get("restored_items", {}).get("containers"):
            ...
            for container in containers:
                print(f"  {container.get('name')} ({container.get('id')}) - {container.get('status')}")
