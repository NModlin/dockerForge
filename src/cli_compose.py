...
@click.option("--path", "-p", multiple=True, help="Paths to search for Docker Compose files")
...
def list_compose_files(obj, recursive, path, format):
    ...
    if format == "json":
        ...
        for file_path, file_info in compose_files.items():
            ...
            import datetime
            last_modified = datetime.datetime.fromtimestamp(file_info.last_modified).strftime("%Y-%m-%d %H:%M:%S")
...
def up_compose_file(obj, file, service, detach, build, validate):
    ...
    success, output = obj.compose_operations.up(file, services=services, detach=detach, build=build)
...
@click.option("--remove-orphans", is_flag=True, default=True, help="Remove containers for services not defined in the Compose file")
@click.pass_obj
def down_compose_file(obj, file, volumes, remove_orphans):
    ...
    # Stop services
    success, output = obj.compose_operations.down(file, volumes=volumes, remove_orphans=remove_orphans)
...
@click.option("--controlled", "-c", is_flag=True, help="Perform controlled restart with health checks")
@click.pass_obj
def restart_compose_file(obj, file, service, controlled):
    ...
    if controlled:
        ...
        success, health_status = obj.compose_operations.controlled_restart(file, services=services)
...
@click.option("--tail", "-n", default="all", help="Number of lines to show from the end of the logs")
@click.pass_obj
def logs_compose_file(obj, file, service, follow, tail):
    ...
    services = list(service) if service else None
    success, output = obj.compose_operations.logs(file, services=services, follow=follow, tail=tail)
...
def exec_compose_file(obj, file, service, command):
    ...
    logger.info(f"Executing command in Docker Compose service {service} from {file}: {command_str}")
...
@click.argument("file", type=click.Path(exists=True))
@click.option("--backup", "-b", help="Backup file to compare with (defaults to latest backup)")
@click.pass_obj
    ...
    if format == "json":
        ...
        for template_name in templates:
            ...
            result.append({
                ...
                "variables": list(obj.template_manager.extract_template_variables(template)),
            })
        click.echo(json.dumps(result, indent=2))
    else:
...
def create_template(obj, file, service, template_name):
    ...
    try:
        ...
        # Create template
        template_path = obj.template_manager.create_template_from_service(compose_data, service, template_name)
...
@click.option("--variable", "-v", multiple=True, help="Template variable in the format name=value")
@click.pass_obj
def apply_template(obj, template_name, output_file, variable):
    ...
    try:
        ...
        compose_data = obj.template_manager.create_compose_from_template(template_name, variables)
...
@click.argument("file", type=click.Path(exists=True))
@click.option("--type", "-t", type=click.Choice(["dependency", "network", "volume", "resource"]), default="dependency", help="Visualization type")
@click.option("--format", "-f", type=click.Choice(["mermaid", "dot", "json"]), default="mermaid", help="Output format")
...
def visualize_compose_file(obj, file, type, format, output):
    ...
    try:
        ...
        if type == "dependency":
            visualization = obj.visualization.generate_dependency_graph(compose_data, output_format=format)
        elif type == "network":
            visualization = obj.visualization.generate_network_graph(compose_data, output_format=format)
        elif type == "volume":
            visualization = obj.visualization.generate_volume_graph(compose_data, output_format=format)
        elif type == "resource":
            visualization = obj.visualization.generate_resource_chart(compose_data, output_format=format)
        
        # Save to file if requested
        if output:
            output_path = obj.visualization.save_visualization(visualization, output, format)
