...
from .platform_detector import PlatformType, InitSystem, get_platform_info
...
class PlatformAdapter(ABC):
    ...
    @abstractmethod
    def execute_command(self, command: List[str], use_sudo: bool = False) -> Tuple[bool, str, str]:
        """
...
class LinuxAdapter(PlatformAdapter):
        ...
        if path == "/var/run/docker.sock" or path == "/run/docker.sock":
            ...
            try:
                ...
                if username:
                    ...
                    if success:
                        return True, f"Added user {username} to docker group. You may need to log out and back in for changes to take effect."
                    else:
    ...
    def execute_command(self, command: List[str], use_sudo: bool = False) -> Tuple[bool, str, str]:
        ...
        try:
            if use_sudo and not self.platform_info.is_root and self.platform_info.has_sudo:
...
class MacOSAdapter(PlatformAdapter):
        ...
        if success:
            ...
            if success:
                ...
                if success:
                    ...
                    return False, f"Failed to start Docker Desktop after killing it: {stderr}"
            else:
    ...
    def start_on_boot(self, enable: bool = True) -> Tuple[bool, str]:
        ...
        if enable:
            cmd = ["defaults", "write", "com.docker.docker", "autoStart", "-bool", "true"]
        else:
            cmd = ["defaults", "write", "com.docker.docker", "autoStart", "-bool", "false"]
...
class WindowsAdapter(PlatformAdapter):
    ...
    def execute_command(self, command: List[str], use_sudo: bool = False, shell: bool = False) -> Tuple[bool, str, str]:
...
def get_platform_adapter() -> PlatformAdapter:
    ...
    if platform_info.platform_type == PlatformType.LINUX:
        ...
        raise NotImplementedError(f"Unsupported platform: {platform_info.platform_type}")
